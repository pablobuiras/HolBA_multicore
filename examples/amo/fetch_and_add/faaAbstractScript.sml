...
Definition faa_aprog_def:
  faa_aprog =
  BirProgram [
      <|bb_label := BFAA_Address_HC (Imm64 0w) "" ;
        ...
        [
          ...
          (* 4w: r1 := r3 + r2 *)
            BStmt_Assign (BVar "_r1" $ BType_Imm Bit64)
              BExp_Add
              (BExp_Load (BExp_Den $ BVar "_r3" $ BType_Mem Bit64 Bit8))
              (BExp_Load (BExp_Den $ BVar "_r2" $ BType_Mem Bit64 Bit8)) ;
        ] ;
        bb_last_statement :=
        BStmt_Jmp (BLE_Label $ BFAA_Address $ Imm64 12w) ;
      |> ;
