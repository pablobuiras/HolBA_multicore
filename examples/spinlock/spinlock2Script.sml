...
Theorem in_crit_before4:
  ...
  ==> ?j st' t. j < i
     ...
     /\ bst_pc_tuple st'.bst_pc = (BL_Address (Imm64 20w),1)
...
(* a successful lr-sc takes the lock *)
(* For a full spinlock program the lr is necessary to distinguish sc from any
 * other later write *)

Theorem lr_sc_takes_lock:
  !cid tr t1 i1 t2 i2 p st.
  wf_trace tr /\ progressing_trace tr
  /\ core_runs_spinlock cid $ FST $ HD tr
  /\ lr_sc cid tr t1 i1 t2 i2
  /\ FLOOKUP (FST $ EL i2 tr) cid = SOME $ Core cid p st
  ==> ?loc st'.
    oEL (PRE t2) (SND $ EL (SUC i2) tr)
      = SOME <| cid := cid; loc := loc; val := lock_taken |>
    /\ bir_eval_view_of_exp (BExp_Den spinlock_var) st.bst_viewenv < t2
    /\ bir_eval_exp (BExp_Den spinlock_var) st.bst_environ = SOME loc
    /\ bir_eval_view_of_exp
        (BExp_Cast BIExp_LowCast (BExp_Const $ Imm64 0w) Bit32)
        st.bst_viewenv < t2
    /\ FLOOKUP (FST $ EL (SUC i2) tr) cid = SOME $ Core cid p st'
    /\ bir_env_update "x6" (BVal_Imm (Imm64 0w)) (BType_Imm Bit64) st.bst_environ
      = SOME st'.bst_environ
Proof
  rpt gen_tac >> strip_tac
  >> fs[lr_sc_def]
  ...
  >> drule_all_then assume_tac wf_trace_core_runs_spinlock_FLOOKUP
  >> dxrule_at_then (Pat `is_fulfil _ _ _ _`) assume_tac is_fulfil_parstep_nice_eq
  >> gvs[FLOOKUP_UPDATE,bir_get_stmt_bir_spinlock_prog_BirStmt_Write_EQ,bst_pc_tuple_def,bir_programTheory.bir_pc_next_def,oEL_THM,bir_eval_exp_view_def]
  >> conj_asm1_tac
  >- (
    qmatch_rename_tac `v = lock_taken`
    >> qpat_x_assum `SOME v = bir_eval_exp _ _` mp_tac
    >> EVAL_TAC
  )
  >> EVAL_TAC
  >> simp[bir_spinlock_prog_def,fulfil_update_env_def,bir_programTheory.bir_programcounter_t_component_equality,AllCaseEqs(),bir_envTheory.bir_var_name_def,bir_envTheory.bir_var_type_def]
QED

(* a successful lr-sc must have read a free lock *)

Theorem lr_sc_fails:
  !cid tr t1 i1 t2 i2 p st loc.
  wf_trace tr /\ progressing_trace tr
  /\ core_runs_spinlock cid $ FST $ HD tr
  /\ lr_sc cid tr t1 i1 t2 i2
  /\ FLOOKUP (FST $ EL i1 tr) cid = SOME $ Core cid p st
  /\ bir_eval_exp (BExp_Den spinlock_var) st.bst_environ = SOME loc
  ==> 0 < t1 /\ mem_read (SND $ EL (SUC i1) tr) loc t1 = SOME lock_free
Proof
  rpt gen_tac >> strip_tac
  >> conj_asm1_tac
  >- (
    cheat (* initially a lock is not taken *)
  )
  >> drule_at_then (Pat `FLOOKUP _ _ = _`) assume_tac wf_trace_core_runs_spinlock_FLOOKUP
  >> `?st' new_env v.
     mem_read (SND (EL i1 tr)) loc t1 = SOME v
     /\ FLOOKUP (FST $ EL (SUC i1) tr) cid = SOME $ Core cid p st'
     /\ env_update_cast64 (bir_var_name (BVar "x5" (BType_Imm Bit64))) v
          (bir_var_type (BVar "x5" (BType_Imm Bit64))) st.bst_environ
          = SOME st'.bst_environ
    /\ bst_pc_tuple st'.bst_pc = (BL_Address $ Imm64 12w, 3)` by (
    fs[lr_sc_def]
    >> drule_at_then (Pat `is_read_xcl _ _ _ _`) assume_tac is_read_xcl_parstep_nice_eq
    >> gvs[is_read_xcl_def,FLOOKUP_UPDATE,bir_get_stmt_bir_spinlock_prog_BirStmt_Read_EQ,bir_programTheory.bir_pc_next_def,oEL_THM,bir_eval_exp_view_def,bst_pc_tuple_def]
  )
  >> cheat
  (*
lr_sc_takes_lock
...
m`sl_step _ _`
       ...
       a0 = (BL_Address (Imm64 12w),3) ∧ a1 = (BL_Address (Imm64 16w),0) ∨
       a0 = (BL_Address (Imm64 16w),0) ∧ a1 = (BL_Address (Imm64 12w),0) ∨
       a0 = (BL_Address (Imm64 16w),0) ∧ a1 = (BL_Address (Imm64 20w),0) ∨
       a0 = (BL_Address (Imm64 20w),0) ∧ a1 = (BL_Address (Imm64 20w),1) ∨
       a0 = (BL_Address (Imm64 20w),1) ∧ a1 = (BL_Address (Imm64 20w),2) ∨
       a0 = (BL_Address (Imm64 20w),2) ∧ a1 = (BL_Address (Imm64 20w),5) ∨

  >> fs[mem_read_thm,mem_read_aux_def,oEL_THM]
  *)
QED

(* lock already taken *)

Theorem lr_sc_pair_locked:
  ...
  /\ t2 <= t1'
  /\ (EL (PRE t1') $ SND $ EL (SUC i2) tr).loc = XX spinlock_var
  /\ (EL (PRE t1') $ SND $ EL (SUC i2) tr).val = lock_taken
  /\ cid <> cid'
...
Proof
  cheat
(*
cid' reads at t1' that the lock is taken,
thus the fulfil transition j2 -> SUC j2 is not possible
*)
QED

(* wlog we have the last lr before the store *)

Theorem lr_sc_without_interleaving_read:
  !tr cid cid' t1 t2 i1 i2 t1' t2' j1 j2.
  wf_trace tr /\ progressing_trace tr
  /\ lr_sc cid tr t1 i1 t2 i2
  ==> ?t1' i1'. lr_sc cid tr t1' i1' t2 i2
    /\ t1 <= t1' /\ i1 <= i1'
    /\ !i. i1 <= i /\ i < i1'
...
Proof
  ...
  >> qmatch_assum_rename_tac `lr_sc cid' tr t1' i1' t2' i2'`
(*
assume there is interleaving read lr-lr-..-sc
lr_sc_without_interleaving_read
*)
  >> Cases_on `t1 < t1' /\ t1' < t2`
  >- (
    fs[]
    >> dxrule_at (Pat `lr_sc _ _ _ _ _ _`) lr_sc_interleaved_pair
    >> rpt $ disch_then $ drule
    >> fs[]
    >> cheat (* lookup location of writes from spinlock program *)
  )
  >> Cases_on `t1 < t2' /\ t2' < t2`
  >- (
    >> dxrule_at (Pat `lr_sc _ _ _ _ _ _`) lr_sc_interleaved_pair'
    >> rpt $ disch_then $ drule
    >> fs[]
  ...
  >> Cases_on `t1 = t1'`
  >- (
    cheat (* impossible by program flow *)
  )
  >> Cases_on `t2 = t2'`
  >- (
    cheat (* impossible as cid is tied to *)
  )
  >> Cases_on `t1 = t2'`
  >- cheat
  >> cheat (* lr_sc_pair_locked *)
QED
